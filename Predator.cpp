...
void Predator::update(sf::Vector2f t_playerPos, double dt)
{
	...
	dir.y = t_playerPos.y - pred.getPosition().y;
	dir = unitVectorify(dir);
	

	pred.setPosition(sf::Vector2f(pred.getPosition().x + dir.x * 125 * (dt / 1000), pred.getPosition().y + dir.y * 125 * (dt / 1000)));
